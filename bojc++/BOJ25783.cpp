...
template <typename T>
T rotating_calipers(vector2d<T>* points, int size)
{
    T dist_max = 0;
    int j = 0;
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        do
        {
            ...
            dist_max = max(dist_max, (points[i] - points[j]).sqr_magnitude());
            if (cross_product(iVector, jVector) > 0)
                j = jNext;
            else
                break;
        } while (true);
...
vector2d<double> planes[100000][3];
...
bool param_search(int n, double x, double d)
{
    for (int i = 0; i < n; ++i)
    ...
    points.push_back(intersection_point(dq.back(), dq.front()));
    for (auto iter = dq.begin(); iter != dq.end() - 1; ++iter)
        points.push_back(intersection_point(*iter, *(iter + 1)));

    return rotating_calipers(points.data(), points.size()) >= d;
}
...
int main()
{
    ...
    double x = 0;
    for (double dx = 1 << 24; dx > 1e-16; dx *= 0.5)
    {
        if (param_search(n, x + dx, d))
            x += dx;
    }
    cout << x;
}