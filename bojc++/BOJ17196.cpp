...
line2d lines[100000];
array<int64, 4> points[200000];
multiset<pair<line2d, int>> intersects;
...
int main()
{
    ...
    for (int i = 0; i < n * 2; ++i)
    {
        ...
        {
            auto iter = intersects.insert(make_pair(lines[points[i][3]], points[i][3]));
            if (next(iter) != intersects.end() && line_intersection(next(iter)->first, iter->first))
            {
                p = make_pair(next(iter)->second, iter->second);
                break;
            }
            if (iter != intersects.begin() && line_intersection(prev(iter)->first, iter->first))
            {
                p = make_pair(prev(iter)->second, iter->second);
                break;
            }
        }
        else
        {
            auto iter = intersects.lower_bound(make_pair(lines[points[i][3]], 0));
            if (next(iter) != intersects.end() && iter != intersects.begin() && line_intersection(prev(iter)->first, next(iter)->first))
            {
                p = make_pair(prev(iter)->second, next(iter)->second);
                break;
            }
    ...
    {
        if (i != p.first)
            c1 += line_intersection(lines[i], lines[p.first]);
        if (i != p.second)
            c2 += line_intersection(lines[i], lines[p.second]);
    }

    if (c1 != c2)
        cout << (c1 > c2 ? p.first : p.second) + 1;
    else
        cout << min(p.first, p.second) + 1;
}